...
export class CensosComponent implements OnInit {
  ...
  constructor(public _censosService:CensosService, private _loteService:LoteService, private activatedRoute: ActivatedRoute) { 
  ...
  filtroEstadoCensos(){
    this.estadoCensos.data = this.censos.filter(censo => {
      return(
        (this.procesoCensos.value.fumigado == null && this.procesoCensos.value.elimando == null && this.procesoCensos.value.pend_fumigar == null) ||
        (censo.estado_censo=== 'FUMIGADO' &&  (this.procesoCensos.value.fumigado)) || 
        (censo.estado_censo === 'ELIMINADO' && (this.procesoCensos.value.eliminado)) ||
        (censo.estado_censo === 'PENDIENTE POR FUMIGAR' && (this.procesoCensos.value.pend_fumigar)) ||
        (!this.procesoCensos.value.fumigado && !this.procesoCensos.value.eliminado && !this.procesoCensos.value.pend_fumigar)
        )
        && 
        (this.range.get('start').value == null || censo.fechaCenso >= this.range.get('start').value) && 
        (this.range.get('end').value == null || (censo.fechaCenso <= this.range.get('end').value)) &&
        (this.procesoCensos.value.nombreLote == censo.nombre_lote || this.procesoCensos.value.nombreLote == "TODOS")
    });
