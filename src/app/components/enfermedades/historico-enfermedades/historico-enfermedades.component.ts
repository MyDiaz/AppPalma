...
export class HistoricoEnfermedadesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.enfermedadesService.getEnfermedadesRegistradas().subscribe(
      (data) => {
        this.historicoEnfermedades = data.map((element) => {
          element.diaRegistroEnfermedad = new Date(
            element.fecha_registro_enfermedad
          );
          element.fecha_registro_enfermedad = moment(
            element.diaRegistroEnfermedad
          )
            .locale("es")
            .format("LL");
    ...
    this.enfermedadesService.getEnfermedadesConcat().subscribe((data) => {
    ...
    if (this.nombreLoteParams != null) {
      this.procesoHistoricoEnfermedades
        .get("nombreLote")
        .setValue(this.nombreLoteParams);
  ...
  filtroEstadoHistoricoEnfermedades() {
    this.estadoHistoricoEnfermedades.data = this.historicoEnfermedades.filter(
      (enfermedad) => {
        ...
        return (
          (this.procesoHistoricoEnfermedades.value.enfermedadConcat ==
            enfermedad.nombre_enfermedad +
              " " +
              (enfermedad.etapa_enfermedad ?? "") ||
            this.procesoHistoricoEnfermedades.value.enfermedadConcat ==
              "TODAS") &&
          (this.range.get("start").value == null ||
            enfermedad.diaRegistroEnfermedad >=
              this.range.get("start").value) &&
          (this.range.get("end").value == null ||
            enfermedad.diaRegistroEnfermedad <= this.range.get("end").value) &&
          (this.procesoHistoricoEnfermedades.value.nombreLote ==
            enfermedad.nombre_lote ||
            this.procesoHistoricoEnfermedades.value.nombreLote == "TODOS")
        );
      }
    );
