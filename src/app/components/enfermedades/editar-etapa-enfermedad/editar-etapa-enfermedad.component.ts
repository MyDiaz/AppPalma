...
export class EditarEtapaEnfermedadComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private router:Router, private activatedRoute:ActivatedRoute,
    ...
    this.enfermedadesService.getEnfermedadEtapas(this.nombre_enfermedad).subscribe( data => {
  ...
  crearEtapasEnfermedadForm(){
    //if(this.nombre_enfermedad)
      this.editarEnfermedadEtapaForm = this.fb.group({
            ids_etapas_enfermedad        : this.fb.array(this.enfermedad_etapas.map(a => a.id_etapa_enfermedad)),
            nombre_enfermedad            : [`${this.nombre_enfermedad}`, [Validators.required, Validators.minLength(3)]],
            etapas_enfermedad            : this.fb.array(this.enfermedad_etapas.map(a => a.etapa_enfermedad)),
            tratamientos_etapa_enfermedad: this.fb.array(this.enfermedad_etapas.map(a => a.tratamientos_etapa_enfermedad))
          });
    /*}else{
      this.editarEnfermedadEtapaForm = this.fb.group({
  ...
  get IDsEnfermedad() {
    return this.editarEnfermedadEtapaForm.get('ids_etapas_enfermedad') as FormArray;
  }
  ...
  get Tratamientos() {
    return this.editarEnfermedadEtapaForm.get('tratamientos_etapa_enfermedad') as FormArray; 
  }

  addEtapa(){
    this.Etapas.push( this.fb.control(''));
      //this.ids_posiciones_puntajes_insertadas.push(-1);
  }

  addTratamiento(){
    this.Tratamientos.push( this.fb.control(''));
  }
  ...
  borrarFila(){
    let i = this.Etapas.length - 1;
    if( i != 0){
      this.Etapas.removeAt(i);
      this.Tratamientos.removeAt(i);
  ...
  actualizarEnfermedad(){
    ...
    let valores_etapas_enfermedad = {
      ids_etapas_enfermedad: this.editarEnfermedadEtapaForm.value.ids_etapas_enfermedad.map( ids => { 
        return ids
      }), 
      nombre_enfermedad: encodeURIComponent(this.editarEnfermedadEtapaForm.value.nombre_enfermedad),
      etapas_enfermedad: this.editarEnfermedadEtapaForm.value.etapas_enfermedad.map( etapas => { 
        return encodeURIComponent(etapas)
      }),
      tratamientos_etapa_enfermedad: this.editarEnfermedadEtapaForm.value.tratamientos_etapa_enfermedad.map( tratamieto => { 
    ...
    Swal.fire({
      ...
      this.enfermedadesService.actualizarEnfermedadConEtapas(this.nombre_enfermedad, valores_etapas_enfermedad).subscribe(
        resp => {
          let rta = resp;
          Swal.fire({
