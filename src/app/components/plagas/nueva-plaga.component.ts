...
export class NuevaPlagaComponent implements OnInit {
  ...
  constructor(
    ...
    this.plagasService.getPlaga(this.nombrePlaga).subscribe(
      (data) => {
        ...
        this.cargando = true;
        this.crearFormularioPlagas();
        console.log(this.NuevaPlagaForm.controls);
      },
      (error) => {
        this.bandera = true;
  ...
  crearFormularioPlagas() {
    if (this.nombrePlaga) {
      ...
      this.NuevaPlagaForm = this.fb.group({
        ids_etapas_plaga: this.fb.array(
          this.plagaEditar.map((a) => a.id_etapa_plaga)
        ),
        nombre_comun_plaga: [
          `${this.nombrePlaga}`,
        ...
        nombre_etapa_plaga: this.fb.array(
          this.plagaEditar.map((a) => a.nombre_etapa_plaga)
        ),
        procedimiento_etapa_plaga: this.fb.array(
          this.plagaEditar.map((a) => a.procedimiento_etapa_plaga)
        ),
      });
    } else {
      //crear
  ...
  get nombreEnfermedadEtapasNoValido() {
    return (
      this.NuevaPlagaForm.get("nombre_comun_plaga").invalid &&
      this.NuevaPlagaForm.get("nombre_comun_plaga").touched
    );
  }
  ...
  guardarPlaga() {
    ...
    let valores_etapas_plaga = {
      nombre_comun_plaga: encodeURIComponent(
        this.NuevaPlagaForm.value.nombre_comun_plaga
      ),
      nombre_etapa_plaga: this.NuevaPlagaForm.value.nombre_etapa_plaga.map(
        (etapas) => {
          return encodeURIComponent(etapas);
        }
      ),
      procedimiento_etapa_plaga:
        this.NuevaPlagaForm.value.procedimiento_etapa_plaga.map(
          (tratamieto) => {
  ...
  actualizarPlaga() {
    ...
    let valores_etapas_plaga = {
      ids_etapas_plaga: this.NuevaPlagaForm.value.ids_etapas_plaga.map(
        (ids) => {
          return ids;
        }
      ),
